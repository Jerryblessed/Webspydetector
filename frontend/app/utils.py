...
def read_config(path) -> any:
    ...
    Args:
        path (tuple): The path as ('category', 'key')
...
def delete_captures() -> bool:
    ...
    try:
        ...
        for line in sh.communicate()[0].splitlines():
            ...
            if res: 
                sp.Popen(["nmcli", "con", "delete", res[0]])

        return True
    except Exception as e:
        print(f"Error deleting captures: {e}")
        return False

def get_battery_level() -> int:
    """Get the battery level. 
       Returns 101 if the power supply is connected or not found.
...
def get_iocs_number() -> int: 
    ...
    Returns:
        int: number of IOCs
    """
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "database.sqlite3")
